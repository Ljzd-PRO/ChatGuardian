...
def _render_matcher_node(root: dict, path: list, sk: str, depth: int = 0) -> None:
    ...
    if mat_type in COMPOUND_TYPES:
        ...
        ctrl_cols = st.columns([3, 2, 2, 6] if (not is_root and not is_not) else (
            [3, 2, 9] if (is_root and not is_not) else ([3, 3, 6] if not is_root else [3, 9])))
        with ctrl_cols[0]:
        ...
        if llm_health:
            ...
            if scheduler:
                with st.expander("⚙️ 引擎调度器运行诊断", expanded=False):
                    ...
                    st.caption(
                        f"当前并发上限: {scheduler.get('max_parallel_batches', '未知')} | 缓存命中数: {metrics.get('idempotency_completed_hits', 0)}")
...
def page_settings():
    ...
    groups = {"LLM 设置": ["llm_"], "检测行为": ["detection_", "context_", "history_", "pending_"],
              "适配器": ["onebot_", "telegram_", "wechat_", "feishu_", "enabled_adapters"], "基础配置": []}
            ...
            with tab:
                for key, field in g_fields:
                    ...
                    if "bool" in annotation:
                        ...
                        new_values[key] = st.number_input(key, value=int(current_val) if current_val is not None else 0,
                                                          step=1)
                    elif "float" in annotation:
                        new_values[key] = st.number_input(key,
