...
def create_app() -> FastAPI:
    ...
    async def auth_login(payload: dict = Body(...)) -> dict[str, object]:
        ...
        Returns:
            dict[str, object]: 字段说明：
                ...
                - ``username``：已认证的用户名。
                - ``using_default_credentials``：是否仍在使用默认账号密码。

        Raises:
            HTTPException: 400 当缺少用户名或密码；401 当凭据错误。
        """
        username = str((payload or {}).get("username") or "").strip()
        password = str((payload or {}).get("password") or "")
        try:
            return await operations.auth_login(username=username, password=password)
    ...
    async def auth_status(request: Request) -> dict[str, object]:
        ...
        Returns:
            dict[str, object]: 字段说明：
                - ``authenticated``：布尔值，恒为 True 代表已通过中间件校验。
                - ``username``：当前登录的用户名。
                - ``using_default_credentials``：是否仍在使用默认账号密码。

        Raises:
            HTTPException: 401 当请求未携带或未通过认证。
        """
        current_user = getattr(request.state, "user", None)
        try:
            return await operations.auth_status(current_user=current_user)
        except OperationError as exc:
            raise _to_http_error(exc) from exc
