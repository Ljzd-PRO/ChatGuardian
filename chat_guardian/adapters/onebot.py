...
class OneBotAdapter(Adapter):
        ...
        async def _on_message(event: dict[str, Any]) -> None:
            try:
                ...
                if chat_event is None:
                    ...
                    return
                logger.debug(f"💬 收到 OneBot 消息 | sender={chat_event.message.sender_id} | chat={chat_event.chat_id} | type={chat_event.chat_type.value}")
    ...
    async def _convert_event(self, event: dict[str, Any]) -> ChatEvent | None:
        try:
            ...
            logger.debug(f"✓ OneBot 事件已转换 | msg_id={message.message_id} | sender={message.sender_id} | from_self={is_from_self}")
            return chat_event
        except Exception as e:
    ...
    async def _build_message(self, event: dict[str, Any], depth: int = 0) -> ChatMessage | None:
        try:
            ...
            logger.debug(f"✓ 消息已构建 | id={msg.message_id} | sender={sender_name or sender_id} | segments={segment_count}")
            return msg
